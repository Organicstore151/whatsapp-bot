...
app.post("/webhook", async (req, res) => {
  ...
  if (session.step === "waiting_for_command") {
    ...
    await client.messages.create({
      to: from,
      messagingServiceSid: process.env.MESSAGING_SERVICE_SID,
      body: "*✍️ Пожалуйста, отправьте список препаратов, которые вы хотите заказать.*_Вы также можете прикрепить фото рецепта или написать названия вручную:_",
    });
    ...
    await client.messages.create({
      to: from,
      messagingServiceSid: process.env.MESSAGING_SERVICE_SID,
      body: "*📦 Пожалуйста, укажите адрес доставки.*_Наш менеджер свяжется с вами в ближайшее время, чтобы подтвердить заказ, уточнить удобное время и способ доставки, а также согласовать оплату:_",
    });
